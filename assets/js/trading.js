...
export default async function trading(slug) {
    if (!sessionStorage.getItem('token')) {
        ...
        let tradingItem = jsonData.lists.filter(item => item.productCode == slug)[0];
        let uri = tradingItem.getLogin;

        tradingLogin(uri, tradingItem);

        async function tradingLogin(uri, tradingItem) {
            ...
            if (isLine()) {
                if (resp.data.code == tradingItem.success) {
